...
reference_file <- "G:/1Yunvjian/0a26.7.7singlecell/8.10/yjsl_RCTD_fullannotation_refinedFib.rds"
spatial_file <- "spatial_PD_input.rda"
result_file <- "RCTD_PD_refinedFib_ECM_results.rds"
preprocessed_file <- "RCTD_PD_refinedFib_ECM_preprocessed.rds"
spatial_output_file <- "stRNA_RCTD_refinedFib_ECM_PD.rds"
weights_file <- "RCTD_PD_refinedFib_ECM_weights.csv"
weights_full_file <- "RCTD_PD_refinedFib_ECM_weights_full.csv"
annotation_counts_file <- "RCTD_PD_refinedFib_ECM_reference_cell_counts.csv"
run_log <- "RCTD_PD_refinedFib_ECM_run.log"
...
ggsave("PD_ECM_Fib_spatial_RCTD.pdf", p_ecm, width = 5.6, height = 5.0)
ggsave("PD_Activated_Fib_spatial_RCTD.pdf", p_activated, width = 5.6, height = 5.0)
...
ggsave("PD_ECM_Fib_region_boxplot_RCTD.pdf", region_plot("ECM_Fib"), width = 6.3, height = 5.2)
ggsave("PD_Activated_Fib_region_boxplot_RCTD.pdf", region_plot("Activated_Fib"), width = 6.3, height = 5.2)

# Use continuous weights_full for composition/enrichment summaries. Restricted
# doublet weights are retained for dominant-type and focal spatial maps.
weights_df <- data.frame(spot_id = rownames(weights_full), weights_full, check.names = FALSE)
...
utils::write.csv(avg_celltype, "RCTD_PD_refinedFib_ECM_region_mean_proportions_full.csv", row.names = FALSE, fileEncoding = "UTF-8")
...
p_stack <- ggplot(avg_celltype, aes(x = Region, y = Mean_proportion, fill = Celltype)) +
  ...
  labs(title = "PD RCTD cell-type composition", x = NULL, y = "Mean proportion", fill = "Cell type")
ggsave("PD_RCTD_refinedFib_ECM_region_composition.pdf", p_stack, width = 8.0, height = 6.0)

avg_mat <- avg_celltype %>%
  pivot_wider(names_from = Region, values_from = Mean_proportion) %>%
  as.data.frame()
...
if (any(!is.finite(row_sd) | row_sd == 0)) {
  stop("Cannot row-scale invariant weights_full rows: ",
       paste(rownames(avg_mat)[!is.finite(row_sd) | row_sd == 0], collapse = ", "))
}
pheatmap(
  avg_mat,
  scale = "row",
  ...
  fontsize_col = 10.5,
  angle_col = 90,
  main = "PD cell-type enrichment",
  filename = "PD_RCTD_refinedFib_ECM_region_heatmap.pdf",
  width = 6.1,
  height = 5.9
)
...
p_violin <- ggplot(composition_long, aes(x = Region, y = Proportion, fill = Region)) +
  ...
  theme(
    axis.text.x = element_text(angle = 55, hjust = 1, size = 7.8),
    strip.text = element_text(face = "bold", size = 10),
    legend.position = "none",
    plot.title = element_text(face = "bold", size = 14)
  ) +
  labs(title = "PD RCTD cell-type proportions across regions", x = NULL, y = "RCTD proportion")
ggsave("PD_RCTD_refinedFib_ECM_region_violin_all.pdf", p_violin, width = 10.2, height = 8.6)
