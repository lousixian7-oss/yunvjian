...
library(ggplot2)
input_dir <- '../TGFb_LR_revision_20260903/inputs'
objects <- setNames(lapply(c('NC','PD'),function(g) readRDS(file.path(input_dir,paste0(g,'_cellchat.rds')))),c('NC','PD'))
...
tables <- lapply(names(objects),function(g) {
  obj <- objects[[g]]
  lr <- obj@LR$LRsig
  lr <- lr[lr$pathway_name=='PERIOSTIN',,drop=FALSE]
  do.call(rbind,lapply(intersect(rownames(lr),dimnames(obj@net$prob)[[3]]),function(k) {
    data.frame(dataset=g,source='Reticular stroma',target=targets,interaction_name=k,
      interaction_name_2=lr[k,'interaction_name_2'],ligand=lr[k,'ligand'],receptor=lr[k,'receptor'],
...
write.csv(all_results,'PERIOSTIN_ReticularStroma_all_tested.csv',row.names=FALSE)
df <- all_results[all_results$passes_nominal_p,]
stopifnot(nrow(df)>0,all(df$ligand=='POSTN'))
write.csv(df,'PERIOSTIN_ReticularStroma_plotted.csv',row.names=FALSE)
print(df);print(table(df$dataset))
df$target <- factor(df$target,levels=targets[targets %in% df$target])
df$dataset <- factor(df$dataset,levels=c('NC','PD'))
# Preserve original K ordering, with ITGB5 above ITGB3.
df$pair <- factor(df$interaction_name_2,levels=sort(unique(df$interaction_name_2)))
...
ggsave('Figure9K_PERIOSTIN_LR_NC_PD.pdf',p,device=cairo_pdf,width=8.9,height=4.45,units='in',bg='white')
saveRDS(p,'Figure9K_PERIOSTIN_LR_plot.rds')
