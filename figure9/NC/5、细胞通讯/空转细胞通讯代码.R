...
group_levels <- c("Basal epithelium" ,  
...
#保存
save(cellchat,file = "cellchatNC.rda")
load("cellchatNC.rda")
cellchatNC<-cellchat
load("cellchatPD_compare5groups.rda")

levels(cellchatNC@idents)
levels(cellchatPD@idents)
...
cellchatNC <- computeCommunProbPathway(cellchatNC)

cellchatNC <- aggregateNet(cellchatNC)
...
interaction_count <- sapply(
  cellchat_merged@net,
  function(x) sum(x$count)
)

interaction_weight <- sapply(
  cellchat_merged@net,
...
p_POSTN_RR <- netVisual_bubble(
  cellchat_merged,
  signaling = "PERIOSTIN",
  sources.use = "Reticular stroma",
  targets.use = c("Reticular stroma","Immune stroma","Basal epithelium" ,  
                  "Suprabasal epithelial" ,  
                  "Inflamed epithelium"
                   ),
  comparison = c(1,2),
