...
yjsl<-readRDS("yjsl_RCTD_fullannotation_refinedFib.rds")
# 查看当前注释
...
common_spots <- intersect(
  rownames(weights),
  rownames(stRNA@meta.data)
)

df <- data.frame(
  Region = stRNA$Region[common_spots],
  PI16_Fib = weights[common_spots, "PI16_Fib"]
)
