...
# 如果 spatial 文件夹在当前工作目录下
tissue_file <- "spatial/tissue_positions_list.csv"
if (file.exists(tissue_file)) {
  tissue_df <- read.csv(tissue_file, header = FALSE, row.names = 1)
  colnames(tissue_df) <- c("in_tissue", "array_row", "array_col", "pxl_row", "pxl_col")
  tissue_spots <- rownames(tissue_df[tissue_df$in_tissue == 1, ])
  cat("组织内 spots 数量:", length(tissue_spots), "\n")
} else {
  stop("找不到 tissue_positions_list.csv，请检查文件路径")
}
stRNA_tissue <- subset(stRNA, cells = tissue_spots)
ncol(stRNA_tissue)   # 应等于 415
stRNA<-stRNA_tissue
...
SpatialPlot(
  stRNA,
  group.by = "seurat_clusters",   # 明确指定按聚类分组
  label = TRUE,
  label.size = 4
)
