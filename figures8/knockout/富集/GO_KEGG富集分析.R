...
pdf("GO-气泡agt敲除.pdf", width =9, height = 4)
print(p)
dev.off()
...
id_mapping <- data.frame(
  ID = c(
    ...
    "KEGG_MEDICUS_REFERENCE_MITOCHONDRIAL_COMPLEX_UCP1_IN_THERMOGENESIS",
  ...
  StandardName = c(
    ...
    "Mitochondrial complex-related metabolism",
    "Translation initiation",
    "Ribosome",
    "DNA replication-associated processes"
  ),
  stringsAsFactors = FALSE
)
# 2. 筛选通路（只保留存在于结果中的）
...
# 6. 保存
pdf("KEGG-气泡图agt敲除.pdf", width = 8, height = 4)
print(p)
dev.off()
